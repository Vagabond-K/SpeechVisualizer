using SpeechVisualizer.Utilities;
using DirectN;
using System;
...
namespace SpeechVisualizer
{
    public class FrameGenerator : IDisposable
    {
        ...
        public FrameGenerator(int port)
        {
            this.port = port;
        }
        ...
        public IComObject<IMFSample> Generate(IComObject<IMFSample> sample, Guid format)
        {
            try
            {
                ...
                #region Vagabond K: 프레임 이미지 수신

                if (_pixelData == null)
                {
                    _pixelData = new byte[_width * _height * 4];
                    _pixelDataPtr = Marshal.UnsafeAddrOfPinnedArrayElement(_pixelData, 0);
                }
                ...
                if (_pixelData != null)
                {
                    ...
                    if (_stream != null)
                    {
                        ...
                        {
                            var received = 0;
                            while (received < _pixelData.Length && !_disposedValue)
                                received += _stream.Read(_pixelData, received, _pixelData.Length - received);
                        }
                        catch
                        {
                            _stream?.Dispose();
                ...
                if (_renderTarget != null && _textFormat != null && _dwrite != null && _whiteBrush != null && _blockBrushes != null)
                {
                    ...
                    #region Vagabond K: 프레임 이미지 그리기

                    using var bufferBitmap = _renderTarget.CreateBitmap(new D2D_SIZE_U(_width, _height), new D2D1_BITMAP_PROPERTIES
                    {
                        pixelFormat = new D2D1_PIXEL_FORMAT
                        {
                            format = DXGI_FORMAT.DXGI_FORMAT_B8G8R8A8_UNORM,
                    ...
                    bufferBitmap.CopyFromMemory(_pixelDataPtr, _width * 4);
                    _renderTarget.DrawBitmap(bufferBitmap,
                        1.0f,
